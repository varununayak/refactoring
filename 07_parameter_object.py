...
def readings_within_range(readings: List[float], min: float, max: float) -> bool:
...
def readings_within_range_refactored(readings: List[float], range : Range) -> bool:
...
if __name__ == "__main__":
    ...
    print(f"Within range: {readings_within_range_refactored(readings, Range(0, 20))}")
    range = RangeBetter(0, 20)
    print(f"Within range: {all([range.within_range(reading) for reading in readings])}")